  ...
  return <section className="panel form">
    ...
    <form action={savePttValidityRuleAction} className="form" key={versionId}>
      ...
      <div className="grid cols-3">
        <div className="field"><label htmlFor="withinMunicipalityDays">Within Municipality</label><NumericInput id="withinMunicipalityDays" name="withinMunicipalityDays" min="1" step="1" defaultValue={selectedRule.withinMunicipalityDays} required integer /></div>
        <div className="field"><label htmlFor="withinProvinceDays">Within Province</label><NumericInput id="withinProvinceDays" name="withinProvinceDays" min="1" step="1" defaultValue={selectedRule.withinProvinceDays} required integer /></div>
        <div className="field"><label htmlFor="withinRegionDays">Within Region</label><NumericInput id="withinRegionDays" name="withinRegionDays" min="1" step="1" defaultValue={selectedRule.withinRegionDays} required integer /></div>
      </div>
      <div className="form-section">
        ...
        {outsideDays.map((days, index) => <div className="grid cols-2 inline-rule-row" key={`${index}-${days}`}>
          <div className="field"><label>Allowed days</label><NumericInput name="outsideRegionAllowedDays" min="1" step="1" value={days} integer onChange={(event) => setOutsideDays((items) => items.map((item, itemIndex) => itemIndex === index ? Number(event.target.value) : item))} required /></div>
        </div>)}
