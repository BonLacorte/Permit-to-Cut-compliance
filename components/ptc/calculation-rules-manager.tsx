    ...
    <form action={savePtcCalculationRuleAction} className="form" key={applicationTypeId || "version-default"}>
      ...
      <div className="form-section"><h3>Processing Fee Tiers</h3>
        {tiers.map((tier, index) => <div className="grid cols-2 inline-rule-row" key={`${index}-${tier.upTo}`}>
          <div className="field"><label>Up to approved trees</label><NumericInput name="tierUpTo" min="1" value={tier.upTo} integer onChange={(event) => setTiers((items) => items.map((item, itemIndex) => itemIndex === index ? { ...item, upTo: Number(event.target.value) } : item))} required /></div>
          <div className="field"><label>Processing fee</label><NumericInput name="tierFee" min="0" step="0.01" value={tier.fee} onChange={(event) => setTiers((items) => items.map((item, itemIndex) => itemIndex === index ? { ...item, fee: Number(event.target.value) } : item))} required /></div>
        </div>)}
      ...
      <div className="grid cols-4">
        <div className="field"><label>Additional tree step</label><NumericInput name="additionalProcessingStep" min="1" defaultValue={selectedRule.additionalProcessingStep} required integer /></div>
        <div className="field"><label>Additional processing fee</label><NumericInput name="additionalProcessingFee" min="0" step="0.01" defaultValue={selectedRule.additionalProcessingFee} required /></div>
        <div className="field"><label>Application fee per tree</label><NumericInput name="applicationFeePerTree" min="0" step="0.01" defaultValue={selectedRule.applicationFeePerTree} required /></div>
        <div className="field"><label>Replanting fee per tree</label><NumericInput name="replantingFeePerTree" min="0" step="0.01" defaultValue={selectedRule.replantingFeePerTree} required /></div>
      </div>
      <div className="form-section"><h3>Validity Rules</h3>
        <div className="field"><label>Maximum trees per individual PTC</label><NumericInput name="maxTreesPerPtc" min="1" defaultValue={selectedRule.maxTreesPerPtc} required integer /></div>
        {brackets.map((bracket, index) => <div className="grid cols-2 inline-rule-row" key={`${index}-${bracket.upTo}`}>
          <div className="field"><label>Up to approved trees</label><NumericInput name="validityUpTo" min="1" value={bracket.upTo} integer onChange={(event) => setBrackets((items) => items.map((item, itemIndex) => itemIndex === index ? { ...item, upTo: Number(event.target.value) } : item))} required /></div>
          <div className="field"><label>Validity days</label><NumericInput name="validityDays" min="1" value={bracket.days} integer onChange={(event) => setBrackets((items) => items.map((item, itemIndex) => itemIndex === index ? { ...item, days: Number(event.target.value) } : item))} required /></div>
        </div>)}
